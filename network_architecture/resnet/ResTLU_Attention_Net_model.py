...
def Conv2dBlock(dim_in, dim_out, kernel_size=3, stride=1, padding=1, bias=True, use_bn=False):
    if use_bn:
        return nn.Sequential(
...
if __name__ == '__main__':
    network = Re_UNet2d(dim_in=3, num_class=3, deep_supervision=True).to("cuda")
    X = torch.rand(12, 3, 192, 192).to('cuda')
    out = network(X)
    for y in out:
        print(y.shape)
