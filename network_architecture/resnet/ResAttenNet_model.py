...
class RA_UNet(nn.Module):
    def __init__(self,
                 dim_in=1, num_conv_block=5, kernel_root=16,
                 use_bn=True):
        ...
        self.skip4 = skip_block(256, 128, 128)
        self.skip3 = skip_block(128, 64, 64)
        self.skip2 = skip_block(64, 32, 32)
        self.skip1 = skip_block(32, 16, 16)

        # maxpool
        self.maxpool = nn.MaxPool2d(2)
        # out_layer
        self.out_layer = nn.Conv2d(kernel_root, 3, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        out = self.skip4(conv_out["conv5"], conv_out["conv4"])
        out = self.skip3(out, conv_out['conv3'])
        out = self.skip2(out, conv_out['conv2'])
        out = self.skip1(out, conv_out['conv1'])
        out = self.out_layer(out)
        return out


if __name__ == '__main__':
    network = Re_UNet2d(dim_in=1).to("cuda")
    X = torch.rand(12, 1, 192, 192).to('cuda')
    out = network(X)
    print(out.shape)