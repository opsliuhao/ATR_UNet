...
def predict_volumes(model, rimg_in=None, cimg_in=None, bmsk_in=None, suffix="pre_mask",
    ...
    for idx, vol in enumerate(volume_loader):
        ...
        for od in range(3):
            ...
            pr_bmsk = torch.zeros([3, len(slice_weight), rescale_dim, rescale_dim])
            if use_gpu:
            ...
            for (i, ind) in enumerate(slice_list):
                ...
                pr_bmsk[:, ind[1], :, :] = pr_bmsk_blk.data[0][:, :, :]
            ...
            if od == 0:
                pr_3_bmsk = torch.unsqueeze(pr_bmsk, 0)
            else:
                pr_3_bmsk = torch.cat((pr_3_bmsk, torch.unsqueeze(pr_bmsk, 0)), dim=0)

        print(pr_3_bmsk[:, :, 256,256,150])
        pr_bmsk = pr_3_bmsk.mean(dim=0)
        print(pr_bmsk[:, 256,256,150])
        # pr_bmsk = pr_bmsk.numpy()
        print(pr_bmsk.max())

        pr_bmsk = torch.argmax(pr_bmsk, dim=0)
        print(pr_bmsk.max(), pr_bmsk.min())
        # pr_bmsk_final = extract_large_comp(pr_bmsk)
        pr_bmsk_final = pr_bmsk.numpy()


        if isinstance(bmsk, torch.Tensor):
            bmsk = bmsk.data[0].numpy()
            dice1, dice2 = estimate_dice(bmsk, pr_bmsk_final)
            if verbose:
                print(dice1, dice2)
